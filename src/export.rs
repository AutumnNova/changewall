...
pub fn export(dict: &ColorDict) -> Result<()> {
	...
	value_vec.push(format!("[{}]{}", dict.alpha, dict.background));
	value_vec.push_variants(dict.foreground.clone());
	value_vec.push_variants(dict.background.clone());
	value_vec.push_variants(dict.cursor.clone());

	for entry in dict.colorvec.to_vec().into_iter() {
